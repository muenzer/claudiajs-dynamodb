module.exports.parse = function(data, conditions) {
  ...
  Object.keys(data).forEach(function(k) {
    ...
    if(data[k]) {
      ...
      if (typeof(data[k]) === 'object') {
        var expressionAtomicKey = '#' + data[k].atomic;
        expressionAttributeNames[expressionAtomicKey] = data[k].atomic

        var expressionValueKey = ':atomic' + data[k].atomic;
        expressionAttributeValues[expressionValueKey] = data[k].value;

        setExpressionStatements.push(expressionNameKey + ' = ' + expressionAtomicKey + ' ' + data[k].operator + ' ' + expressionValueKey);
      }
